    ...
    public sealed class CombatHud : MonoBehaviour
    {
        ...
        private void OnTargetChanged(Targetable target)
        {
            ...
            if (targetPortrait != null)
            {
                ...
                //
                // Порядок: явный ключ, потом таблица по имени, потом свой
                // спрайт.
                //
                // Ключ первым нужен мастям: белые волки зовутся так же, как
                // серые, — просто «Волк», — и таблица выдаёт им серое лицо.
                //
                // А вот «свой спрайт» стоит последним намеренно. Я сначала
                // поднял его на первое место и тем же ходом отнял портрет у
                // НПС: в этом поле у него лежала пустая картинка, снятая
                // когда-то с модели, и она перекрыла таблицу. Пустая строка
                // ключа так навредить не может.
                var art = Portraits.ByKey(target.PortraitKey)
                          ?? Portraits.For(target.DisplayName)
                          ?? target.Portrait;
