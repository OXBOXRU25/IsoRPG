...
namespace IsoRPG.Cameras
{
    ...
    public sealed class IsoCameraRig : MonoBehaviour
    {
        ...
        private void UpdatePlacement(float deltaTime)
        {
            ...
            // Последняя проверка: не оказалась ли камера ВНУТРИ геометрии.
            //
            // Луч выше ловит препятствие между героем и камерой, но не
            // спасает, когда камера уже внутри: физика Unity считает
            // попадание при ВХОДЕ в коллайдер, а изнутри его не видит.
            // Так и выходило на камне: точка старта луча (она догоняет
            // героя с задержкой) оказывалась внутри, и защита слепла.
            //
            // Двигать точку взгляда нельзя — она задаёт кадр, и её скачок
            // сам даёт рывок. Двигаем камеру: подтягиваем её к герою,
            // пока она не выберется наружу.
            for (int step = 0; step < 10; step++)
            {
                if (!Physics.CheckSphere(place, 0.35f, ~0, QueryTriggerInteraction.Ignore)) break;

                Vector3 toHero = lookAt - place;
                if (toHero.sqrMagnitude < 0.04f) break;

                place += toHero.normalized * 0.4f;
            }
