...
namespace IsoRPG.Items
{
    ...
    public sealed class CharacterHud : MonoBehaviour, IsoRPG.UI.IHudWindow
    {
        ...
        /// </summary>
        /// <remarks>
        /// Шаг обязан быть ЦЕЛЫМ. Делением высоты на восемь выходило 60.5, и
        /// каждое второе гнездо вставало на полупиксель: кант рисовался то в
        /// два пикселя, то в три, вдоль всей колонки. Павлон 03.09.2026:
        /// «пляшет обводка, где-то 2 пикселя, где-то 3». Масштаб холста при
        /// этом ровно единица (тянемся за шириной, экран 1920), то есть
        /// логический пиксель равен экранному — виновата была только дробь.
        /// Восемь шагов по 60 занимают 480 против прежних 484: четыре
        /// пикселя запаса внизу колонки глазом не читаются, а ровный кант
        /// читается сразу.
        /// </remarks>
        private const float SlotStep = 60f;
        ...
        private void BuildRow(RectTransform parent, EquipSlot slot, float x, float y)
        {
            ...
            float side = slot == EquipSlot.Ammo ? Mathf.Round(Slot * 0.82f) : Slot;
            float sink = Mathf.Round((Slot - side) * 0.5f);

            // Гнездо садится на целый пиксель — и позицией, и размером.
            //
            // Кант рисуется подложкой, растянутой ровно на пиксель наружу:
            // встань край гнезда на половину пикселя, и этот пиксель размажет
            // между двумя, а соседнее гнездо на целой координате нарисует его
            // честно. Отсюда шла пляшущая обводка. Округляем ЗДЕСЬ, а не в
            // раскладке: тогда правило накрывает все гнёзда сразу — и колонки,
            // и полосу оружия, и то, что добавят позже, — а не те три, что
            // попались под руку.
            iconRect.anchoredPosition = new Vector2(Mathf.Round(x + sink), Mathf.Round(y - sink));
            ...
            // Отступ ЦЕЛЫЙ. Здесь стояло 1.5 — и это была настоящая причина
            // пляшущего канта, которую я два раза искал не там.
            //
            // Видимая кромка гнезда складывается из двух вещей: рамка
            // выступает на пиксель наружу, а картинка отходит внутрь на этот
            // отступ. Полтора плюс один — это два с половиной пикселя, и
            // округляются они по-разному с разных сторон: слева и сверху
            // вниз, справа и снизу вверх. Отсюда ровно то, что видел Павлон
            // 04.09.2026: «верхняя и левая одной толщины, правая и нижняя
            // шире». Замер по его кропу: 2, 2, 3, 3 — и с широкой стороны
            // средний пиксель приглушён, то есть край лёг на половину.
            //
            // С целой единицей кромка выходит ровно два пикселя со всех
            // четырёх сторон и не зависит ни от позиции окна, ни от стороны.
