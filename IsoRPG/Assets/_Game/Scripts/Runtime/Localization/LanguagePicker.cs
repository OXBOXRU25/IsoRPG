...
namespace IsoRPG.Localization
{
    ...
    public sealed class LanguagePicker : MonoBehaviour
    {
                                            ...
                                            float width, float height = 30f)
        {
            ...
            // Подписываемся ЗДЕСЬ, а не только в Awake.
            //
            // AddComponent на активном объекте вызывает Awake немедленно —
            // строкой выше, когда кнопок ещё нет. Подписка в Awake находила
            // ноль кнопок и молча заканчивалась ничем, поэтому в окне
            // настроек переключатель не работал вовсе.
            //
            // В главном меню он работал: там компонент и кнопки лежат в
            // сцене, и к моменту Awake всё уже на месте. Из-за этого разлад
            // выглядел как «в меню язык меняется, в игре нет».
            picker.Subscribe();
