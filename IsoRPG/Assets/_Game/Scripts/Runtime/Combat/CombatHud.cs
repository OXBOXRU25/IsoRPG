    ...
    public sealed class CombatHud : MonoBehaviour
    {
        ...
        private void OnTargetChanged(Targetable target)
        {
            ...
            if (targetPortrait != null)
            {
                ...
                //
                // Сначала СВОЙ портрет цели, потом таблица по имени. Порядок
                // был обратный, и это ломалось на мастях: белые волки зовутся
                // так же, как серые, — просто «Волк», — поэтому таблица
                // выдавала им серое лицо, а назначенное белое до экрана не
                // доходило вовсе. Павлон 03.09.2026: «белым волкам поставил
                // серый портрет».
                //
                // Общее правило: то, что назначено конкретному существу,
                // главнее того, что выведено из его имени.
                var art = target.Portrait ?? Portraits.For(target.DisplayName);
