...
namespace IsoRPG.Cameras
{
    ...
    public sealed class IsoCameraRig : MonoBehaviour
    {
        ...
        ///
        /// Растянуть его я сперва попробовал до 2.5 — и Павлон сразу поймал:
        /// «растворение начинается уже издалека, а должно — когда мы почти
        /// вплотную». Он прав, лечить надо было не диапазон, а шаг колеса:
        /// теперь у упора он мелкий (см. <see cref="ZoomStepAt"/>), и на эти
        /// 0.8 приходится три-четыре щелчка вместо одного.
        ///
        /// Считать надо так: диапазон в долях размера, а не в делениях.
        ...
        /// <summary>
        /// Шаг колеса — доля текущего приближения, а не постоянное число.
        ///
        /// Постоянный шаг в единицу хорош вдалеке и слишком груб вблизи: у
        /// самого упора одно деление колеса меняет вид вдвое, и всё, что
        /// должно происходить постепенно, проскакивает между двумя щелчками.
        /// Именно на это налетело растворение героя: оно живёт на последнем
        /// метре приближения, а метр этот проходился одним щелчком.
        ///
        /// Доля от размера даёт привычное поведение: далеко — размашисто,
        /// вплотную — мелко. Так же ведёт себя колесо в WoW, и по той же
        ...
        private void ReadZoomInput()
        {
            ...
            desiredOrthoSize = Mathf.Clamp(
                desiredOrthoSize - Mathf.Sign(scroll) * ZoomStepAt(desiredOrthoSize),
                minOrthoSize,
                maxOrthoSize);
        }
