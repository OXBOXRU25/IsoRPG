    ...
    public static class PendingTasks
    {
        ...
        private static void Execute(string task)
        {
            ...
            switch (lowered)
            {
                ...
                // Иконки нижнего ряда — отдельным заданием.
                //
                // 03.09.2026 две новые иконки, карта и гильдия, встали в ряд
                // чёрными квадратами. Виноваты не картинки: задание «hud»
                // вешает компоненты и картинок не касается, а разметку PNG в
                // спрайт делает только `player-kit`, которого в тот вечер не
                // гоняли. Файлы приехали обычными текстурами, а запрос
                // спрайта по пути к такой текстуре возвращает пусто — молча,
                // без единого предупреждения. Отдельное задание нужно затем,
                // чтобы ради семи картинок не гонять весь набор игрока.
                // Свой указатель мыши вместо системной стрелки.
                // Примерка анимаций клавишами F1..F4 — инструмент приёмки.
                // Настоящие скорости всех беговых клипов — таблицей в журнал.
                // Манекен для проверки боевых анимаций.
                // Консоль анимаций: окно со списком всех клипов, показ на манекене.
                case "anim-console":
                    AnimConsoleKit.Apply();
                    break;
                ...
                case "hud-icons":
                    {
                        ...
                        var bar = pl.GetComponent<IsoRPG.UI.HudBar>();
                        if (bar == null) bar = pl.AddComponent<IsoRPG.UI.HudBar>();

                        SandboxSceneBuilder.SetupHudBar(bar);
                        UnityEditor.SceneManagement.EditorSceneManager.MarkAllScenesDirty();

                        Debug.Log("[IsoRPG] Иконки нижнего ряда розданы заново.");
                    }
                    break;
