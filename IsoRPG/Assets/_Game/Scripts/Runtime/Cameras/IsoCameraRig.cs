...
namespace IsoRPG.Cameras
{
    ...
    public sealed class IsoCameraRig : MonoBehaviour
    {
        ...
        private void UpdatePlacement(float deltaTime)
        {
            ...
            // внутри кроны.
            // Точку проверки вынимаем из геометрии, если она внутри.
            //
            // Она берётся от СГЛАЖЕННОГО положения героя и догоняет его с
            // задержкой: запрыгнул на камень — герой наверху, а точка ещё
            // внутри камня. Луч, пущенный изнутри коллайдера, его не видит,
            // защита слепнет, и камера проходит насквозь. Павлон 01.09.2026:
            // «запрыгиваю на камень, разворачиваюсь — и на долю секунды
            // текстуры на весь экран».
            for (int lift = 0; lift < 8; lift++)
            {
                if (!Physics.CheckSphere(lookAt, 0.3f, ~0, QueryTriggerInteraction.Ignore)) break;
                lookAt += Vector3.up * 0.35f;
            }
            ...
            if (reach > 0.1f)
            {
                ...
                if (wanted > 0.01f)
                {
                                           ...
                                           ~0, QueryTriggerInteraction.Ignore))
                    {
                        ...
                        float safe = Mathf.Max(hit.distance - 0.45f, 1f);
