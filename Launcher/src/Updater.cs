...
namespace HighFlyingBird.Launcher
{
    ...
    ///
    /// Обновляется только папка игры. Себя лаунчер перезаписать не может —
    /// файл занят запущенным процессом, и Windows этого не позволит. Поэтому
    /// сам лаунчер меняется вместе с установщиком, а не на лету.
    ///
