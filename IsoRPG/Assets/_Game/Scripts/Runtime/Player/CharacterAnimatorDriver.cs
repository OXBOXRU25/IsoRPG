...
namespace IsoRPG.Player
{
    ...
    public sealed class CharacterAnimatorDriver : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            // Втрое, а не вдвое: Павлон 04.09.2026 «при приземлении персонаж
            // едет по земле до перехода на бег». Так и есть — приседание с
            // разгибанием занимают почти секунду, и на бегу этого времени
            // хватает на пару метров скольжения. Стоя оставляем как есть:
            // там ехать некуда, и быстрое разгибание выглядит дёрганьем.
            animator.SetFloat(LandSpeedHash, smoothedSpeed > 0.15f ? 3f : 1f);
