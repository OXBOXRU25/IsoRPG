...
namespace IsoRPG.Cameras
{
    ...
    public sealed class IsoCameraRig : MonoBehaviour
    {
        ...
        ///
        /// Стояло 0.6 — и это МЕНЬШЕ одного шага колеса, который у нас равен
        /// единице. Растворение целиком проскакивало между двумя делениями:
        /// одно — герой почти целый, следующее — его уже нет. Павлон
        /// 04.09.2026: «не плавное, буквально два состояния — лёгкая
        /// прозрачность и сразу полное исчезновение».
        ///
        /// Два с половиной деления дают три-четыре различимых ступени, и
        /// приближение читается как проход сквозь героя, а не как его
        /// выключение. Больше брать нельзя: герой начнёт таять там, где он
        /// ещё нужен целым.
        /// </summary>
        private const float FadeZoomSpan = 2.5f;
        ...
        private void FadeHero(float distance)
        {
            ...
            //
            // Одной дистанции мало. Павлон 03.09.2026: «в WoW, приближая
            // вплотную, герой тает до полной прозрачности, и дальше смотришь
            // сквозь него, а у нас он виден в любом случае». Так и было:
            // на самом близком делении камера не доходила до MinReach, и
            // растворение застывало где-то на середине — герой висел
            // полупрозрачным и всё равно закрывал обзор.
            //
            // Считаем по зуму отдельно и берём меньшее из двух. Тогда
            // прозрачность гарантирована и упором зума, и близкой камерой —
