...
namespace IsoRPG.Player
{
    ...
    public sealed class KeyboardMove : MonoBehaviour
    {
        ...
        /// и никуда не едет. Раньше здесь спрашивалась фактическая скорость
        /// капсулы, а она в упоре ровно ноль, и герой замирал столбом. В WoW
        /// он в этом положении перебирает ногами на месте, и Павлон разобрал
        /// это по своим кадрам 03.09.2026: побежал в лоб — топчется, повернул
        /// камеру — пошёл вдоль преграды. Скольжение вдоль даёт сама капсула,
        /// от нас нужны только ноги.
        /// </remarks>
        public float Speed => !steering ? 0f
                             : agent != null ? agent.speed
