        ...
        private bool TryPickTarget(Vector2 screenPosition)
        {
            ...
            foreach (var hit in hits)
            {
                ...
                if (shop != null)
                {
                    ...
                    {
                        // Не бежим — как в WoW. Нажатие на собеседника его
                        // ВЫДЕЛЯЕТ, и только: подложка под ногами и портрет
                        // сверху уже показаны выше. Решение Павла 03.09.2026:
                        // левая кнопка обязана значить одно, а бегает игрок
                        // сам. Немого отказа при этом нет — говорим словами.
                        IsoRPG.Combat.CombatLog.Add("Слишком далеко — подойди ближе.");
                        return true;
                    }
                ...
                if (giver != null)
                {
                    ...
                    {
                        // Далеко — только выделяем, бежать не заставляем.
                        // Подложка и портрет уже показаны, а дойти игрок
                        // решает сам: левая кнопка значит одно.
                        IsoRPG.Combat.CombatLog.Add("Слишком далеко — подойди ближе.");
                    }
